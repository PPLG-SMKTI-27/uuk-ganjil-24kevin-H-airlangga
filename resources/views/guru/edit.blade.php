...
@section('subtitle', 'Perbarui data guru yang sudah ada')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center space-x-2 text-sm">
        ...
        <span class="text-gray-600">Edit Guru</span>
    </div>

    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <!-- Header -->
        <div class="bg-gradient-to-r from-primary-600 to-purple-600 px-8 py-8 text-white">
            <h2 class="text-3xl font-bold mb-2">Edit Data Guru</h2>
            <p class="text-blue-100">Perbarui informasi guru {{ $guru->nama_guru }}</p>
        </div>

        <!-- Form Body -->
        <div class="p-8">
            <form action="{{ route('guru.update', $guru->id_guru) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')

                <!-- Form Section: Data Pribadi -->
                <div>
        ...
        <!-- Info Box -->
        <div class="bg-amber-50 border-t-2 border-amber-200 px-8 py-6">
            <div class="flex items-start">
                <i class="fas fa-exclamation-triangle text-amber-600 text-xl mt-1 mr-4"></i>
                <div>
                    <h4 class="font-semibold text-amber-900 mb-2">Catatan Penting</h4>
                    <ul class="text-sm text-amber-800 space-y-1">
                        <li>• Perubahan NIP akan mempengaruhi referensi kunjungan yang ada</li>
                        <li>• Pastikan perubahan data sudah benar sebelum menyimpan</li>
                        <li>• Jika ada kesalahan, perubahan dapat diulang kapan saja</li>
                    </ul>
                </div>
            </div>
        </div>
